...
- Accept the public inputs required to count tokens: plain-text `input_data` plus `model_ref`, or a provider-native typed request object for Anthropic, Gemini, or OpenAI when `provider_id` matches, together with `provider_id` and `api_key`.
...
The module exposes one public function and works with three public request models:

- `calc_tokens(...) -> TokenCount`: dispatch plain-text requests to any provider and dispatch `AnthropicCountTokensRequest`, `GeminiCountTokensRequest`, or `OpenAICountTokensRequest` only to their matching providers.
- `TokenCount(total: int)`: normalized response model for the public API. It also exposes `TokenCount.from_anthropic(...)`, `TokenCount.from_gemini(...)`, and `TokenCount.from_openai(...)` for typed provider paths.
...
- `TypeError`: the caller passed an `AnthropicCountTokensRequest`, `GeminiCountTokensRequest`, or `OpenAICountTokensRequest` with a mismatched provider id, or mixed a request object with `model_ref`.
...
- Adding a new provider should not require changing `calc_tokens`; only the registry and provider implementation need to change, unless that provider also gets a typed request model.
